  ...
  return (
    <div className="mx-auto max-w-xl space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon-sm" onClick={() => navigate('/users')} aria-label="Quay lại">
          <ArrowLeft className="size-4" />
        </Button>
        <div>
          <h1 className="page-header">{isEdit ? 'Chỉnh sửa người dùng' : 'Tạo người dùng'}</h1>
          <p className="page-subheader">{isEdit ? `Đang sửa: ${current?.fullName ?? '…'}` : 'Điền thông tin để tạo tài khoản mới'}</p>
        </div>
      </div>
        ...
        {isLoadingUser ? (
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-5">
            ...
            <Field label={isEdit ? 'Mật khẩu mới (để trống nếu không đổi)' : 'Mật khẩu'} required={!isEdit} error={errors.password}>
              <Input type="password" placeholder={isEdit ? '••••••••  (không bắt buộc)' : '••••••••'} value={form.password} onChange={set('password')} autoComplete="new-password" />
            </Field>
